...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。**分区徽章布局**: 宠物 (眼球追踪/碎碎念▸) → 显示 (HUD 面板) → 行为 (窗口/指令/模式) → 视觉 (特效) → 玩法 (娱乐/提醒/分身) → 系统 (自启动/休息)。每个分区用彩色左边框全宽徽章标识，去除所有 Emoji |
| `context_menu.gd` | 菜单 UI 逻辑。**双面板架构**: 主菜单 + 信息侧栏 (日期/实时时钟/WiFi)。**智能四象限定位** + 信息栏自动贴靠主菜单侧边。**分区标题样式化**: 动态徽章 (PanelContainer + 彩色左边框)。**区块色系自动继承**: 子菜单边框色自动匹配触发按钮所属分区色。碎碎念为单选子菜单 (关闭/30分钟/60分钟)。WiFi 查询通过 WorkerThreadPool 异步执行，过滤 `Wi-Fi\|WLAN\|Wireless` 接口 |
