...
| `context_menu.gd` | **菜单主控制器**。方向感知三级定位 (`_menu_side` 驱动弹出方向)。提供全菜单统一的面板外边缘定位与「动态指示三角形护盾」辅助渲染服务（`get_panel_bounds_for_button`, `draw_panel_tail`）。通过 5 个 RefCounted 子系统委托：侧栏、气泡、子菜单、浮动提示、特效预览 |
