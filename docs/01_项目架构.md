...
| `eye_behavior.gd` | ~284 | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。**强制注视方向**: `forced_look_dir` 由状态机设定，优先级最高。**休眠模式**: `drowsy_amount` 控制持续半闭状态 (lerp 过渡)，休眠态抑制正常眨眼。**休眠视觉参数**: `hibernate_dim` (亮度衰减) 和 `hibernate_iris_shrink` (光圈收缩) 供扩展风格使用。**扫描模式**: 1.2s 内 2 次快速左右来回 (smoothstep 急停转向)，完成后回调。鼠标空闲时间 `_mouse_idle_time` 供休眠触发判断。按需重绘 |
...
| `context_menu.gd` | 菜单主控制器 (~870行)。**双面板架构**: 主菜单 + 信息侧栏。**智能四象限定位**。通过 5 个 RefCounted 子系统委托: `_sidebar` (侧栏), `_sysinfo_bubble` (系统气泡), `_submenu` (级联子菜单), `_tooltip` (浮动提示), `_styler` (分区样式)。**胶囊按钮**: 面板入口 (外观主题/提醒管理) 和退出按钮使用 `_apply_capsule_style()` 添加半透明背景+细边框+圆角的独特视觉样式 |
| `context_menu/info_sidebar.gd` | **信息侧栏** (RefCounted, ~176行)。日期/时钟/WiFi/电池/开机时长。WiFi+电池合并一次 PS 调用，开机时长本地计算零开销。`WorkerThreadPool` 异步执行 |
| `context_menu/sysinfo_bubble.gd` | **系统信息气泡** (RefCounted, ~275行)。终端扫描报告风格，RichTextLabel + BBCode 五段着色。**身份段** (HOST/USER/OS/DISP 青色) + **硬件段** (CPU+核心基频副行/GPU/MEM/MOBO/MON/IP 蓝色) + **存储段** (磁盘竖排, 三级颜色) + **运行时段** (BOOT/UP/BAT 紫色, 电池三级颜色)。显示器独立 WMI 查询带容错。IP 过滤虚拟网卡。开机时间 Godot 端反推+时区修正。视口边界钳制 + 弹簧动画 |
...
9. [休眠] 机械挡板 (draw_colored_polygon + draw_polyline AA 轮廓, 弧形超出巩膜 1px 消除内切弦间隙)
