...
| `pet_gaming.gd` | **游戏态管理器** (RefCounted)。管理游戏态全部行为：宠物锁定 (高阻尼+清零速度)、悬浮踏板升降 (StaticBody2D 单向碰撞)、瞳孔强制注视全息屏、SubViewport 纹理梯形透视渲染 (近端收缩 15%)。启动时取消空间跳跃，退出时设置 roam 冷却 |
...
| 组件 | 文件 | 职责 |
|------|------|------|
| BaseGame 基类 | `games/base_game.gd` | 定义 `start()` / `cleanup()` 接口 + `game_finished` 信号，管理解构式悬浮 UI (标题气泡/侧边按钮/重开按钮) + 全息合成视口 |
...
游戏 UI 采用解构式布局（面板 + 悬浮组件分离），BaseGame 创建独立的全息合成视口，将游戏面板纹理 + 悬浮组件克隆体合成为完整画面。PetGaming 通过 `get_holo_texture()` 获取合成纹理，自动显示完整的 UI 缩小版。
...
| 全息迷你屏 | 固定尺寸 (2.0R×2.5R), 合成视口纹理梯形透视渲染 (近端收缩 15%), 理想位置布局不受面板拖拽影响 |
