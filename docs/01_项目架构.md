...
| 文件 | 行数 | 职责 |
|------|------|------|
| `event_bus.gd` | ~45 | **全局事件总线**。声明了 19 个信号用于模块间解耦通信，覆盖拖拽、状态变化、UI 控制、系统功能、窗口模式、行为指令、克隆系统、颜色系统 (`pet_color_changed`/`ui_theme_changed`/`show_theme_panel`/`pet_scanning_changed`) 等类别。持有 `ui_hue` 全局可读变量 (UI 主题色) |
...
| `pet.gd` | ~830 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 6 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)、`IdleBehaviors` (待机微行为)。**休眠挡板渲染**: 通过圆弧段多边形绘制上方机械挡板，实现半闭眼效果。**自定义休眠动画框架**: 支持替换渲染模式 (暗屏覆盖 + 旋转加载指示器/电池图标)。**检索动画**: 系统信息查询时显示加载旋转器，查询完成后交叉混合过渡为绿色对勾图标。**瞳孔图标覆盖系统**: 支持 5 种独立图标 (信封/感叹号/问号/对勾/交叉)，通过 `pet_show_eye_icon` 信号触发，覆盖到白边框内侧 (0.83R)，图标态抑制眨眼。**空间跳跃系统** (`free_roam`): 宠物先跳 → 到达抛物线顶点时透明踏板出现在脚下接住 → 落稳 0.8s 后继续跳 (2~4 级) → 电梯式缓降归位。攀升期间锁定 idle 状态+关闭状态机转换。踏板为 `StaticBody2D` 单向碰撞+科技风能量光线渲染 (`_PlatformVisual` 内部类: 从中心展开动画+刻度线+能量粒子流动+着陆闪光环)。着陆制动 (`_roam_brake`) 防滚落。电梯下降: 关闭重力+踏板 100px/s 匀速下移+宠物刚性位置锁定+距地 20px 踏板渐隐+重力恢复。拖拽中断调用 `_roam_finish()` 统一清理。指令序列触发支持 `"hibernate:N"` 格式解析。所有 draw_circle 均启用抗锯齿 |
...
| `idle_behaviors.gd` | ~234 | **Idle 微行为管理器** (RefCounted)。管理待机状态下的 2 种微行为子阶段: **低功耗休眠** (鼠标静止 5 分钟触发，`update()` 主动持续监控不依赖 idle 状态机，覆盖自由行动/安静待命两种模式。默认风格: 机械挡板半闭眼 + 瞳孔下垂 + 呼吸节律起伏。**无限期休眠**: 只有鼠标活动才能唤醒，最低持续 5 秒防误触。唤醒时必定发送一条话术。**多风格支持**: 预留加载指示器/电池图标等自定义动画框架。身体回正: 进入休眠时停止角速度并 lerp 旋转归零。本轮离席仅触发一次) / **系统自检** (真实时钟驱动，每 60~90 分钟一次，3小时内最多 3 次)。分身不运行。可被拖拽/戳一戳打断 |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时根据步态风格调整 Walk/Jump 概率: 蹦跳为主 85%/15%、滚动为主 100%/0%、混合平衡 90%/10%。注视同伴时不跳动。散步者接近时被动跳开让路。安静模式下精确匹配槽位坐标。**微行为集成**: 通过 `idle_behaviors.try_random()` 触发自检，活跃时阻止状态转移，exit 时 `cancel()` 清理。休眠触发已迁移至 `idle_behaviors.update()` 主动监控。**空间跳跃集成**: `_roam_active` 时 process/physics_process 全部直接 return，锁定 idle 状态不转换，由 roam 系统完全接管物理。自动触发: `free_roam_enabled` 且 idle_timer 到期时 6% 概率调用 `_start_free_roam()` |
...
| `drag.gd` | 弹簧力公式 `F = k·x - c·v` 让宠物弹性跟随鼠标，同时降低重力影响。松手时判定「戳一戳 (Poke)」(拖拽 < 0.25s + 移动 < 12px) 委托至 `pet.handle_poke()` 优先级路由系统。**空间跳跃中断保护**: enter 时检测 `_roam_active` 并调用 `_roam_finish()` 统一清理攀升/下降状态 |
| `fall.gd` | 自由落体/上升 (尊重 `gravity_sign` 恢复正确的 `gravity_scale`)，进入时清除残留的 `forced_look_dir`。稳定后转 Idle。安静模式下落地后自动发起全局阵列重排，被拖拽时显示科技风归位话术 (随机10选1，带实时距离/计数，防重复) |
...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。**分区徽章布局**: 宠物 (眼球追踪/碎碎念) → 显示 (HUD 面板) → 行为 (窗口/指令/模式) → 视觉 (特效/外观主题) → 玩法 (娱乐/提醒/分身) → 系统 (系统信息/自启动) → 指令序列 (待机/诊断/确认) → 休息。**视觉规范**: 开关项右侧 `[●]`/`[○]`、子菜单右侧 `[+]`、面板入口胶囊按钮 (半透明背景+细边框+圆角)、退出红色胶囊 |
| `context_menu.gd` | 菜单主控制器 (~965行)。**双面板架构**: 主菜单 + 信息侧栏。**方向感知四象限定位**: `_menu_side` 全局方向状态驱动主菜单/侧栏/子菜单/tooltip 的弹出方向和动画锚点。通过 5 个 RefCounted 子系统委托: `_sidebar` (侧栏), `_sysinfo_bubble` (系统气泡), `_submenu` (级联子菜单), `_tooltip` (浮动提示), `_styler` (分区样式)。**胶囊按钮**: 面板入口 (外观主题/提醒管理) 和退出按钮使用 `_apply_capsule_style()` 添加半透明背景+细边框+圆角的独特视觉样式 |
...
| `reminder_panel.gd` | **提醒管理面板** (CanvasLayer, ~690行)。自适应高度卡片列表 (hover 高亮+删除淡出动画)、自定义拨轮时间选择器 (▲/当前值/▼ + 鼠标滚轮 + 上下拖拽, 分钟 5 分钟步进)、`1x单次`/`↻ 每日` 胶囊切换、回车提交。**UI 主题色同步**: 监听 `ui_theme_changed` 实时更新面板边框/标题/分隔线/拨轮底板/类型标签。外部点击关闭时 `set_input_as_handled()` 防穿透。一次性提醒触发后自动删除，每 10 秒检查系统时间 |
