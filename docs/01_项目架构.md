...
| `pet_holo_screen.gd` | **全息屏渲染控制器** (RefCounted)。管理全息迷你屏的梯形透视渲染 (近端收缩 15% + 微后仰)、投影支架线绘制、屏幕矩形计算 (供面板定位避让)、动态间距自适应 (游戏贴近/终端拉开，lerp 平滑过渡)。从 pet_gaming.gd 抽离，支持多模式个人终端扩展 |
...
| 模式抢占 | `show_game()` 自动清理 IDLE/LOADING 模式的资源 |
