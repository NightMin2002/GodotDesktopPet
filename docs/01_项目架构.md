...
| `clone_pet.gd` | ~32 | **克隆分身控制器**，继承 `pet.gd`。覆写 2 个方法实现功能裁剪。克隆体不显示 HUD 面板 |
...
| `walk.gd` | **步态风格驱动移动**：蹦跳为主 → 纯冲量跳跃；滚动为主 → 100% 短距滚动 (120~300px，扭矩 15000，前方 80px 同伴避让)；混合平衡 → 50%/50%。**滚动散步特殊事件** (独立开关): 8% 概率触发长距滚动到对面边缘，推开前方 140px 内同伴让路。含 0.8s 卡死检测 |
...
| 滚动散步 | `stroll` | `pet.stroll_enabled` (8% 特殊事件开关) | ✅ ConfigFile |
