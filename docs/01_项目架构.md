...
| `context_menu.gd` | 菜单主控制器 (~949行)。**双面板架构**: 主菜单 + 信息侧栏。**智能四象限定位**。通过 5 个 RefCounted 子系统委托: `_sidebar` (侧栏), `_sysinfo_bubble` (系统气泡), `_submenu` (级联子菜单), `_tooltip` (浮动提示), `_styler` (分区样式)。**胶囊按钮**: 面板入口 (外观主题/提醒管理) 和退出按钮使用 `_apply_capsule_style()` 添加半透明背景+细边框+圆角的独特视觉样式 |
