...
| `pet_effects.gd` | ~200 | **视觉特效管理器** (RefCounted，从 pet.gd 拆分)。管理冲击波、全息拖影和能量共鸣弧。**双配色模式**: 虹彩 (彩虹循环, 默认) / 跟随体色 (纯色)。**能量共鸣弧**: 多宠物距离<150px时自动绘制锯齿状闪电弧，**缓存路径架构** (`_arc_paths` 字典 + `_arc_regen` 0.07s 周期重生)，避免每帧随机拖动。双色渐变辉光+白色内核，`_arc_flash` 统一闪烁衰减。颜色通过 `palette.effective_hue()` 联动 |
