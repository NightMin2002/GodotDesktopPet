...
| `eye_behavior.gd` | ~98 | **瞳孔行为控制器** (继承 `RefCounted`，不是 Node)。**全部在世界坐标系中计算**，不受刚体旋转影响。管理两种瞳孔模式: 鼠标追踪 (12x 速率，世界空间方向) → 好奇游走 (鼠标静止 2.5s 后自主漫游/追踪关闭时始终游走，70% 概率随机看、30% 回看中心)。同时管理机械虹膜眨眼动画: 每 2.5~7s 触发一次，20% 概率连眨 |
...
| `walk.gd` | **小蹦跳 (Hop)**：一次性向上+水平冲量 (高度 180~350, 水平 80~220)，轻微自然旋转 (2000~6000 扭矩冲量)。落地稳定后回到 Idle。靠近屏幕边缘自动往回跳 |
