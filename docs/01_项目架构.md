...
| `eye_behavior.gd` | ~273 | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。**强制注视方向**: `forced_look_dir` 由状态机设定，优先级最高。**休眠模式**: `drowsy_amount` 控制持续半闭状态 (lerp 过渡)，休眠态抑制正常眨眼。**扫描模式**: 1.2s 内 2 次快速左右来回 (smoothstep 急停转向)，完成后回调。鼠标空闲时间 `_mouse_idle_time` 供休眠触发判断。按需重绘 |
| `idle_behaviors.gd` | ~215 | **Idle 微行为管理器** (RefCounted)。管理待机状态下的 2 种微行为子阶段: **低功耗休眠** (鼠标静止 5 分钟触发，机械挡板半闭眼 + 瞳孔下垂 + 休眠气泡，持续阶段最低 5 秒后方响应鼠标唤醒，本轮离席仅触发一次) / **系统自检** (真实时钟驱动，每 60~90 分钟一次，3小时内最多 3 次)。分身不运行。可被拖拽/戳一戳打断 |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时根据步态风格调整 Walk/Jump 概率: 蹦跳为主 85%/15%、滚动为主 100%/0%、混合平衡 90%/10%。注视同伴时不跳动。散步者接近时被动跳开让路。安静模式下精确匹配槽位坐标。**微行为集成**: 通过 `idle_behaviors.try_random()` 触发休眠/自检，活跃时阻止状态转移，exit 时 `cancel()` 清理 |
