...
│   │   ├── sec_pet.gd             ← 宠物分区 (碎碎念/分身/训练数据/个人终端/待办清单)
...
│   └── pet_profile_panel.gd       ← 装置档案面板 (CanvasLayer, layer=-1。宠物渲染在面板之上)。独立窗口，宠物身份+游戏战绩+能力数据 (含等级控制、互动按钮)。打开时创建单向碰撞墙围栏 (one_way_collision)，宠物可进入但无法逃出，拖拽时围栏跟随移动
│
