...
| 文件 | 行数 | 职责 |
|------|------|------|
| `event_bus.gd` | ~37 | **全局事件总线**。声明了 15 个信号用于模块间解耦通信，覆盖拖拽、状态变化、UI 控制、系统功能 (含 `force_show_bubble` 强制气泡中断)、窗口模式切换、行为指令切换、克隆系统 (clone_pet/dismiss_clones) 等类别。所有信号发射和监听通过 `EventBus.信号名` 进行 |
...
| `eye_behavior.gd` | ~180 | **瞳孔行为控制器** (继承 `RefCounted`)。**全部在世界坐标系中计算**，不受刚体旋转影响。管理三种瞳孔模式: 鼠标追踪 (12x 速率) → 社交注视 (10% 主动看同伴，35% 回看正在看自己的同伴，含视线遮挡检测防“电灯泡”) → 好奇游走 (70% 随机看、30% 回中心)。同时管理机械虹膜眨眼动画: 每 2.5~7s 触发，20% 概率连眨 |
