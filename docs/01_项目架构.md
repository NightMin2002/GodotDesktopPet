...
| `main.gd` | 脚本 | **项目最核心的脚本 (~970 行)**，承担了以下所有职责：透明窗口初始化 → 屏幕边界墙创建 → 宠物实例化 → DWM 鼠标穿透管理 (Win32 CombineRgn 多矩形区域，气泡覆盖层作为独立矩形) → 幽灵墙系统 (Win32 窗口感知) → 窗口交互三模式 (FREE/CONFINED/REPELLED) → 行为指令系统 (自由/安静待命) → 克隆分身系统 (召唤/遣散/持久化) → 安静待命排队 (基于宠物直径的精确间距 + reorganize_quiet_queue 动态槽位分配) → 提醒系统挂载 → 系统托盘 → 仪式感告别退出 (克隆体依次告别滚出 + 本体最后退场) |
