...
| `hit_region_manager.gd` | ~240 | **DWM 穿透管理器** (从 main.gd 拆分)。**双模式架构**: 启动时尝试 `InjectLayeredStyle()` 注入 `WS_EX_LAYERED`，成功则进入**完美穿透模式** (本地圆形+矩形命中检测 + `WS_EX_TRANSPARENT` 切换，零视觉裁剪)；失败则回退**SetWindowRgn 模式** (AABB 矩形包围盒 + 4px 栅格量化 + 变化检测)。拖拽/菜单状态时全窗口可交互 |
| `clone_manager.gd` | ~270 | **克隆与行为管理器** (从 main.gd 拆分)。管理克隆体生命周期 (召唤/遣散)、安静待命排队 (基于宠物直径的精确间距 + `reorganize_quiet_queue` 动态槽位分配)、行为指令切换。**遣散和告别退出采用相同的仪式感动画**: 克隆体依次说告别语 → 滚向屏幕边缘 → 旋转淡出消失，间隔0.35s依次退场 |
...
| `pet.gd` | ~540 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、全息拖影/冲击波特效系统、科幻单眼渲染。维护行为指令状态和克隆标记。设置开关: `shockwave_enabled` / `trail_enabled` / `stroll_enabled`，均通过 `EventBus.setting_toggled` 信号同步。**戳一戳交互系统**: 优先级路由 (未确认提醒 → 未确认碎碎念 → 连戳递进吐槽 → 久别重逢 → 普通话术) |
| `clone_pet.tscn` | — | 克隆分身预制体。与 `pet.tscn` 结构一致，但挂载 `clone_pet.gd` 脚本 |
| `clone_pet.gd` | ~34 | **克隆分身控制器**，继承 `pet.gd`。覆写 4 个方法实现功能裁剪：`_ready()` 设置 `is_clone` 标记、`_unhandled_input()` 恢复拖拽但屏蔽右键菜单、`_init_hud_clock()` 隐藏 HUD 时钟、`_on_setting_toggled()` 响应通用视觉设置 (冲击波/尾流/散步) 但忽略原体专属功能。克隆体完全自治，拥有独立的状态机和物理碰撞 |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时 85% 转 Walk / 15% 触发 Jump。注视同伴时不跳动 (检测 `eye_behavior._look_at_pet`)。散步者接近时被动跳开让路 (140px 检测范围，冲量 -350，0.5s 冷却)。安静模式下精确匹配 `retreat_target_x` 槽位坐标 (`_is_at_slot()`)，持续 lerp 微校正物理漂移 (速率8，约0.3秒归位)，高阻尼 (linear=5, angular=8) 锁定位置 |
| `walk.gd` | **双模式移动**：92% 蹦跳 (冲量 300~500) / 8% 悠闲散步 (受 `stroll_enabled` 开关控制，扭矩 25000 + 阻尼 1.5 缓速滚动到对面边缘)。散步时通知前方 140px 内同伴跳开让路 (冲量 -280，转矩 ±800，1% 概率轻推彩蛋 + 冲击波)。含 0.8s 卡死检测 |
...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。分组布局: 宠物功能开关 (眼球追踪/全息时钟/碎碎念) → 模式子菜单 (🌍 窗口模式 ▸ / 🏃 行为指令 ▸) → 功能子菜单 (✨ 视觉特效 ▸ / 🎮 娱乐玩法 ▸ / ⏰ 提醒) → 分身操作 → 系统级 (自启动/退出) |
| `context_menu.gd` | 菜单 UI 逻辑。实现弹性跟随宠物 (Lerp)、Q 弹展开/收缩动画、自定义浮动 Tooltip。**级联子菜单系统**: 支持开关型 (`_create_submenu`) 和单选型 (`_create_radio_submenu`) 两种模式，hover 150ms 延时弹出 + 300ms 延时关闭 + 鼠标二次验证。窗口模式子菜单 (自由漫游/窗口封闭/窗口排斥)、行为指令子菜单 (自由行动/安静待命)、视觉特效子菜单 (冲击波/尾流)、娱乐玩法子菜单 (滚动散步)。单选子菜单带 tooltip 描述。碎碎念三态、外部点击关闭 (含子菜单区域排除) |
