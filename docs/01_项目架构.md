...
| `main.gd` | 脚本 | **项目最核心的脚本 (~950 行)**，承担了以下所有职责：透明窗口初始化 → 屏幕边界墙创建 → 宠物实例化 → DWM 鼠标穿透管理 (Win32 CombineRgn 多矩形区域，气泡覆盖层作为独立矩形) → 幽灵墙系统 (Win32 窗口感知) → 窗口交互三模式 (FREE/CONFINED/REPELLED) → 行为指令系统 (自由/安静待命) → 克隆分身系统 (召唤/遣散/持久化) → 提醒系统挂载 → 系统托盘 → 告别退出动画 (force_show_bubble 强制气泡 + 输入屏蔽) |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时 85% 转 Walk / 15% 触发 Jump。注视同伴时不跳动 (检测 `eye_behavior._look_at_pet`)。散步者接近时被动跳开让路 (跳绳效果)。安静模式下检测边缘并触发 Retreat |
...
| `retreat.gd` | 手动安静待命触发时，读取全局分配的专属槽位坐标（防交叠排队机制）并减速停靠。遇到物理障碍时，0.5秒内基于绝对位移精准触发防卡死保护，就地转化为 Idle。撤退期间支持鼠标左键拖拽打断 (松手后通过 fall→retreat 自动回归队列) |
