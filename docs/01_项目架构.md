...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。顶部日期 Label + ◉/○ 开关按钮、窗口模式三态切换、**级联子菜单触发按钮** (✨ 视觉特效 ▸ / 🎮 娱乐玩法 ▸)、提醒管理入口、克隆/遣散/退出按钮 |
| `context_menu.gd` | 菜单 UI 逻辑。实现弹性跟随宠物 (Lerp)、Q 弹展开/收缩动画、自定义浮动 Tooltip、**级联子菜单系统** (`_build_submenus` 工厂模式构建，hover 150ms 延时弹出 + 300ms 延时关闭 + 鼠标位置二次验证防误关，右侧弹出/空间不足自动左侧)。视觉特效子菜单含冲击波/尾流开关，娱乐玩法子菜单含滚动散步开关。窗口模式三态循环、碎碎念三态、开机自启动、告别退出、外部点击关闭 (含子菜单区域排除) |
