...
| `clone_pet.gd` | ~32 | **克隆分身控制器**，继承 `pet.gd`。覆写 4 个方法实现功能裁剪：`_unhandled_input()` 恢复拖拽但屏蔽右键菜单、`_init_hud_clock()` 隐藏 HUD 时钟、`_on_setting_toggled()` 忽略原体专属设置。克隆体完全自治，拥有独立的状态机和物理碰撞 |
...
| `retreat.gd` | 手动安静待命触发时，读取全局分配的专属槽位坐标（防交叠排队机制）并减速停靠。遇到物理障碍时，0.5秒内基于绝对位移精准触发防卡死保护，就地转化为 Idle。 |
...
| `reminder_bubble.gd` | 气泡通知 (CanvasLayer)。宠物头顶弹出金色边框消息气泡，弹簧动画弹入 → 6 秒后淡出上飘。支持消息队列 (最多缓存 3 条，间隔 1 秒依次播放)、`force_show_bubble` 强制中断 (协程安全取消 `_show_generation`)。气泡面板 `mouse_filter=IGNORE` 鼠标穿透。`overlay_rect` 由 `main.gd` 作为独立矩形注册到 DWM，非定向气泡始终跟随原体 |
...
| 文件 | 职责 |
|------|------|
| `WindowsManager.cs` | 通过 `[DllImport]` 绑定 `user32.dll`、`dwmapi.dll`、`kernel32.dll` 和 .NET `Registry` API。提供核心方法: **GetVisibleWindowRects()** (枚举窗口 + 隐形桌面/UWP覆盖层严格黑名单 + `WS_EX_TOOLWINDOW` 工具层过滤 + DWMWA_CLOAKED 过滤 + Z-Order 遮挡剔除) / **HideFromTaskbar()** (推 WS_EX_TOOLWINDOW) / **BoostProcessPriority()** / **IsAutoStartEnabled() + SetAutoStart()** / **SetWindowRegion()** (DWM 多矩形/多边形穿透合成) |
...
| `show_targeted_bubble` | message: String, target: Node2D | drag.gd (poke) / fall.gd (吐槽) → reminder_bubble.gd (定向到指定宠物) |
...
| 覆盖层独立穿透 | `main.gd` + `reminder_bubble.gd` | 气泡 `overlay_rect` 作为独立矩形注册到 DWM (不与宠物本体合并)，气泡面板 `mouse_filter=IGNORE` | 气泡可见但不阻挡桌面点击，宠物和气泡间隙可穿透 |
