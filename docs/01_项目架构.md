...
│   ├── event_bus.gd               ← 全局事件总线 (~35 行)
│   ├── settings_manager.gd       ← 持久化设置管理器 (~58 行)
│   ├── clone_manager.gd          ← 克隆与行为管理器 (~120 行)
│   ├── farewell_manager.gd       ← 告别退出 + 退场动画 (~173 行)
│   ├── ghost_wall_manager.gd     ← 幽灵墙管理器 (~270 行)
│   └── hit_region_manager.gd     ← DWM 穿透管理器 (~276 行)
│
├── entities/                       ← 实体层
│   └── pet/                       ← 宠物模块
│       ├── pet.tscn               ← 宠物预制体 (RigidBody2D + CircleShape2D)
│       ├── pet.gd                 ← 宠物核心控制器 (~310 行)
│       ├── poke_system.gd         ← 戳一戳交互系统 (RefCounted, ~171 行)
│       ├── pet_hud.gd             ← 本地气泡管理器 (RefCounted, ~100 行)
│       ├── hud_panel.gd           ← 统一 HUD 悬浮面板 (RefCounted, ~245 行)
│       ├── pet_effects.gd         ← 视觉特效管理器 (RefCounted, ~98 行)
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `event_bus.gd` | ~36 | **全局事件总线**。声明了 14 个信号用于模块间解耦通信，覆盖拖拽、状态变化、UI 控制、系统功能 (含 `force_show_bubble` 强制气泡中断)、窗口模式切换、行为指令切换、克隆系统 (clone_pet/dismiss_clones) 等类别 |
| `settings_manager.gd` | ~57 | **持久化设置管理器**。基于 Godot 的 `ConfigFile` API，将用户偏好和提醒数据以 INI 格式保存到 `user://settings.cfg`。**类型分离存储**: bool 存 `switches` section、int 存 `values` section、提醒存 `reminders` section |
...
| `hit_region_manager.gd` | ~276 | **DWM 穿透管理器** (从 main.gd 拆分)。**双模式架构**: 完美穿透模式 (WS_EX_TRANSPARENT) / SetWindowRgn 回退模式。通过 `pet_hud`/`pet_effects` 子系统访问时钟、拖影、冲击波数据 |
| `clone_manager.gd` | ~120 | **克隆与行为管理器** (从 main.gd 拆分)。管理克隆体生命周期 (召唤)、安静待命排队 (基于宠物直径的精确间距 + X/Y 双轴目标适配反重力)、行为指令切换。召唤气泡使用 `force_show_bubble` 避免快速连召时气泡队列堆积 |
...
| `pet.gd` | ~310 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 5 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)。设置开关通过 `EventBus.setting_toggled` 同步到各子系统。**反重力系统**: `gravity_sign` 符号乘数统一控制所有垂直冲量方向 |
...
| `pet_hud.gd` | ~100 | **本地气泡管理器** (RefCounted)。管理本地定向气泡 (每宠物独立堆叠，最多3条，弹簧弹入+淡出上飘)。时钟已迁移至 `hud_panel.gd` 统一管理 |
| `hud_panel.gd` | ~245 | **统一 HUD 悬浮面板** (RefCounted)。实时系统信息卡片，悬浮在宠物侧边。当前组件: 系统时钟 + WiFi SSID 显示。WiFi 通过 `WorkerThreadPool` 后台查询零卡顿。点击 WiFi 行直接打开 Windows 网络设置。智能定位: 宠物在右半屏→面板放左边。右键菜单打开时自动淡出避让。各组件独立开关，全部关闭时面板自动隐藏 |
| `pet_effects.gd` | ~98 | **视觉特效管理器** (RefCounted，从 pet.gd 拆分)。管理冲击波和全息拖影的数据生命周期和渲染委托。未来新特效在此扩展 |
| `clone_pet.tscn` | — | 克隆分身预制体。与 `pet.tscn` 结构一致，但挂载 `clone_pet.gd` 脚本 |
| `clone_pet.gd` | ~29 | **克隆分身控制器**，继承 `pet.gd`。覆写 2 个方法实现功能裁剪。克隆体不显示 HUD 面板 |
...
| WiFi 异步查询 | `hud_panel.gd` | `WorkerThreadPool.add_task()` 后台执行 PowerShell 查询，主线程读取结果 | 零卡顿获取系统信息，不阻塞 120fps 渲染 |
...
pet.gd (核心控制器 ~310 行)
  ...
  ├── hud_panel.gd    (统一 HUD 面板, RefCounted, ~245 行)
...
**设计原则**：核心控制器 + 5个 RefCounted 子系统。子系统均通过 `xxx.pet = self` 注入宿主引用。HUD 面板内各组件独立开关，全部关闭时面板自动隐藏。克隆体不显示 HUD 面板。
