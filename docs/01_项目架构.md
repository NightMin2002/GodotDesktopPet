...
| `ghost_wall_manager.gd` | ~270 | **幽灵墙管理器** (从 main.gd 拆分)。每 0.1s 通过 C# 层获取 Win32 窗口矩形，根据当前模式建墙: FREE (顶/底单向踏板 + Z-Order 遮挡裁剪，反重力时自动翻转踏板方向) / CONFINED (多边形合并封闭) / REPELLED (多边形合并排斥)。监听 `anti_gravity` 设置变更实时切换 |
...
| `pet.gd` | ~630 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、全息拖影/冲击波特效系统、科幻单眼渲染。维护行为指令状态和克隆标记。设置开关: `shockwave_enabled` / `trail_enabled` / `stroll_enabled` / `anti_gravity`，均通过 `EventBus.setting_toggled` 信号同步。**反重力系统**: `gravity_sign` 符号乘数统一控制所有垂直冲量方向，切换时翻转 `gravity_scale` 并给初始推力。气泡/时钟定位根据反重力状态自动切换上方/下方。**戳一戳交互系统**: 优先级路由 (未确认提醒 → 未确认碎碎念 → 连戳递进吐槽 → 久别重逢 → 普通话术) |
| `clone_pet.tscn` | — | 克隆分身预制体。与 `pet.tscn` 结构一致，但挂载 `clone_pet.gd` 脚本 |
| `clone_pet.gd` | ~36 | **克隆分身控制器**，继承 `pet.gd`。覆写 4 个方法实现功能裁剪：`_ready()` 设置 `is_clone` 标记、`_unhandled_input()` 恢复拖拽但屏蔽右键菜单、`_init_hud_clock()` 隐藏 HUD 时钟、`_on_setting_toggled()` 响应通用设置 (冲击波/尾流/散步/反重力) 但忽略原体专属功能。克隆体完全自治，拥有独立的状态机和物理碰撞 |
...
| `walk.gd` | **双模式移动**：92% 蹦跳 (冲量 300~500) / 8% 悠闲散步 (受 `stroll_enabled` 开关控制，扭矩 25000 + 阻尼 1.5 缓速滚动到对面边缘)。散步时通知前方 140px 内同伴跳开让路。所有垂直冲量和扭矩均乘以 `gravity_sign` 适配反重力。含 0.8s 卡死检测 |
