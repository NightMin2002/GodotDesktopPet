...
| `context_menu.gd` | 菜单 UI 逻辑。**智能四象限定位**: 根据宠物屏幕位置自动选择弹出方向。弹性跟随 (Lerp) + Q 弹动画。**分区标题样式化**: 动态将 ▌ 标题替换为 PanelContainer 徽章 (彩色左边框+深背景)。**区块色系自动继承**: 子菜单边框色自动匹配触发按钮所属分区色。级联子菜单系统: 开关型/单选型，hover 延时 + 鼠标二次验证 |
