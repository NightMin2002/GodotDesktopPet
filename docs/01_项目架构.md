...
| `pet.gd` | ~1000 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 6 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)、`IdleBehaviors` (待机微行为)。**休眠挡板渲染**: 通过圆弧段多边形绘制上方机械挡板，实现半闭眼效果。**自定义休眠动画框架**: 支持替换渲染模式 (暗屏覆盖 + 旋转加载指示器/电池图标)。**检索动画**: 系统信息查询时显示加载旋转器，查询完成后交叉混合过渡为绿色对勾图标。**瞳孔图标覆盖系统**: 支持 5 种独立图标 (信封/感叹号/问号/对勾/交叉)。**空间跳跃系统** (`free_roam`): 宠物跳→顶点时踏板接住→落稳后概率决策 (40%继续跳/25%踏板上横移/25%跳下破碎/10%电梯)。**屏幕自适应**: `_screen_scale()` 基准 1080p，所有力度/距离/速度乘缩放因子。**踏板横移**: phase 4 在踏板两端添加空气墙防掉落，踏板每帧 lerp 跟随宠物 X 位置，0.4s 保护期防误判。**跳下破碎**: 瞳孔看向跳跃方向→0.3s蓄力→空气墙立即禁用→起跳(200~350横向冲量)→踏板因冲击破碎(闪白+6片Polygon2D碎片飞散+旋转+渐隐)。**电梯下降**: 关重力+踏板匀速下移+宠物刚性锁定+距地 2R 渐隐+重力恢复。**踏板生命周期**: 3.5s 定时销毁，但宠物在场时自动延期 (2s 重检)。拖拽中断调用 `_roam_finish()` 统一清理。指令序列触发支持 `"hibernate:N"` 格式解析。所有 draw_circle 均启用抗锯齿 |
