...
│   ├── context_menu.gd            ← 右键菜单逻辑 (弹性跟随 + 开关 + 模式切换 + 系统信息)
...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。**分区徽章布局**: 宠物 (眼球追踪/碎碎念▸) → 显示 (HUD 面板) → 行为 (窗口/指令/模式) → 视觉 (特效) → 玩法 (娱乐/提醒/分身) → 系统 (系统信息/自启动/休息)。每个分区用彩色左边框全宽徽章标识 |
| `context_menu.gd` | 菜单 UI 逻辑。**双面板架构**: 主菜单 + 信息侧栏 (日期/实时时钟/WiFi/电池/开机时长)。**智能四象限定位** + 信息栏自动贴靠主菜单侧边。**分区标题样式化**: 动态徽章 (PanelContainer + 彩色左边框)。**区块色系自动继承**: 子菜单边框色自动匹配触发按钮所属分区色。**系统信息气泡**: 点击后宠物气泡异步查询 CPU/GPU/RAM/Disk，带确认按钮关闭。**双层监控架构**: 轻量侧栏 (WiFi+电池一次 PS 查询，开机时长本地计算零开销) + 重量级信息按需触发 (系统信息气泡) |
