...
| `todo_panel.gd` | **待办清单面板** (CanvasLayer)。屏幕 70% 的独立窗口。主从双栏布局：左栏待办列表 / 右栏备注编辑器。所有视觉样式委托 `TodoThemeBase` 及其子类，面板零硬编码颜色 |
| `todo_themes/todo_theme_base.gd` | **待办面板主题基类** (TodoThemeBase, RefCounted)。种子色推算系统: `_from_seeds(base, text, accent, danger, alpha)` 从 5 个参数推算 ~34 个配色属性。提供中性默认组件工厂，子类通过覆写 `make_xxx()` / `apply_xxx()` 实现独立风格。新增骨架组件时只需在基类加中性默认，已有主题零改动 |
