...
| `pet.gd` | ~521 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 6 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)、`IdleBehaviors` (待机微行为)。**休眠挡板渲染**: 通过圆弧段多边形绘制上方机械挡板，实现半闭眼效果。弧形采样 32 段 + 半径微超出巩膜圆 1px 消除内切弦间隙透出 + 同色 draw_polyline 抗锯齿轮廓线。弧线方向修正: span < 0 时加 TAU 避免挡板翻转。高光反射点随 drowsy 程度淡出。**自定义休眠动画框架**: 支持替换渲染模式 (暗屏覆盖 + 旋转加载指示器/电池图标)，平滑淡入淡出。**检索动画**: 系统信息查询时瞳孔区域显示加载旋转器，查询完成后交叉混合过渡为绿色对勾图标，点击确认后淡出恢复。覆盖范围到白边框内侧 (0.83R)，检索期间抑制眨眼。调试触发支持 `"hibernate:N"` 格式解析风格参数。所有 draw_circle 均启用抗锯齿 |
...
| `eye_behavior.gd` | ~330 | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。**强制注视方向**: `forced_look_dir` 由状态机设定，优先级最高。**休眠模式**: `drowsy_amount` 控制持续半闭状态 (lerp 过渡)，休眠态抑制正常眨眼。**休眠视觉参数**: `hibernate_dim` (亮度衰减) 和 `hibernate_iris_shrink` (光圈收缩) 供扩展风格使用。**扫描模式**: 1.2s 内 2 次快速左右来回 (smoothstep 急停转向)，完成后回调。**检索动画**: `scanning_blend` (加载覆盖层) + `scanning_done_blend` (对勾混合度) 双参数交叉混合，检索/完成期间抑制眨眼。提供 `start_scanning()` / `finish_scanning()` / `stop_scanning()` 接口。鼠标空闲时间 `_mouse_idle_time` 供休眠触发判断。按需重绘 |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时根据步态风格调整 Walk/Jump 概率: 蹦跳为主 85%/15%、滚动为主 100%/0%、混合平衡 90%/10%。注视同伴时不跳动。散步者接近时被动跳开让路。安静模式下精确匹配槽位坐标。**微行为集成**: 通过 `idle_behaviors.try_random()` 触发自检，活跃时阻止状态转移，exit 时 `cancel()` 清理。休眠触发已迁移至 `idle_behaviors.update()` 主动监控 |
...
| `context_menu.gd` | 菜单主控制器 (~939行)。**双面板架构**: 主菜单 + 信息侧栏。**智能四象限定位**。通过 5 个 RefCounted 子系统委托: `_sidebar` (侧栏), `_sysinfo_bubble` (系统气泡), `_submenu` (级联子菜单), `_tooltip` (浮动提示), `_styler` (分区样式)。**胶囊按钮**: 面板入口 (外观主题/提醒管理) 和退出按钮使用 `_apply_capsule_style()` 添加半透明背景+细边框+圆角的独特视觉样式 |
...
11. [休眠] 机械挡板 (draw_colored_polygon + draw_polyline AA 轮廓, 弧形超出巩膜 1px 消除内切弦间隙)
