...
| 组件 | 文件 | 职责 |
|------|------|------|
| BaseGame 基类 | `games/base_game.gd` | 定义 `start()` / `cleanup()` 接口 + `game_finished` 信号，GameManager 通过属性注入 SubViewport/Container/屏幕大小 |
...
func start() -> void              # 启动 (构建 UI 到 game_viewport)
...
游戏面板渲染到 SubViewport，pet.gd 通过 `SubViewport.get_texture()` 自动获取缩小版纹理并绘制到宠物侧面。任何游戏只需实现 `start()` 构建 UI，全息屏自动工作，零额外代码。
...
pet.gd _draw():
  └── draw_polygon(梯形顶点, UV, SubViewport.get_texture())
      实时镜像 + 梯形透视 (近宠物端收缩 15%)
```
...
| 全息迷你屏 | SubViewport 纹理自动镜像 + 梯形透视 (近端收缩 15%)，宽高比首帧固定防跳变 |
