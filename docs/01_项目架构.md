...
| `pet.gd` | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染 (Canvas 绘图 API)。通过 8 个 RefCounted 子系统委托职责：`PokeSystem`、`PetHUD`、`HudPanel`、`PetEffects`、`EyeBehavior`、`IdleBehaviors`、`FreeRoamSystem`、`PetGaming`。支持屏幕穿越双重渲染幽灵副本 |
...
| `eye_behavior.gd` | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。支持强制注视方向 (`forced_look_dir`，由状态机/全息屏设定，最高优先级)、休眠模式 (半闭眼)、扫描模式、检索动画、通用图标覆盖 (mail/alert/question/error)。好奇观察：即使追踪关闭，偶尔盯一眼鼠标 |
...
| `pet_gaming.gd` | **游戏态管理器** (RefCounted)。管理游戏态全部行为：宠物锁定 (高阻尼+清零速度)、悬浮踏板升降 (StaticBody2D 单向碰撞)、瞳孔强制注视全息屏。启动时激活 PetHoloScreen 并取消空间跳跃，退出时设置 roam 冷却。全息屏渲染已委托给 PetHoloScreen |
...
| `idle.gd` | 待机后按步态风格概率转 Walk/Jump。安静/深夜模式下精确匹配槽位坐标。集成微行为触发和空间跳跃入口。深夜休眠不被状态转换打断。进入时延迟 0.3s 清零 `forced_look_dir`，让跳跃落地后瞳孔方向自然过渡 |
| `walk.gd` | 步态风格驱动移动：蹦跳为主→纯冲量跳跃、滚动为主→短距滚动、混合→各半。自主巡航特殊事件 (独立开关)。先看方向再行动 (0.3s 缓冲)，蹦跳或滚动期间瞳孔持续看向移动方向 |
| `drag.gd` | 弹簧力跟随鼠标。松手时判定「戳一戳」(短拖+小位移) 委托至优先级路由。进入时清零 `forced_look_dir` (被抓住时恢复自然追踪)。安静/深夜模式拖离记录标记 |
| `fall.gd` | 自由落体/上升，稳定后转 Idle。安静/深夜模式落地后自动发起全局阵列重排 |
| `jump.gd` | 大跳：先看方向 (0.3s) 再施加高抛物线冲量 + 空中翻滚，全程瞳孔注视跳跃方向，靠近边缘自动往回跳 |
| `retreat.gd` | 安静待命时先看向目标方向 (0.3s) 再滚动归位。读取全局槽位坐标并减速停靠，到达后高阻尼刹车。深夜模式下临时解锁旋转确保物理滚动归位 |
