...
│       ├── pet.gd                 ← 宠物核心控制器 (~300 行)
│       ├── poke_system.gd         ← 戳一戳交互系统 (RefCounted, ~171 行)
│       ├── pet_hud.gd             ← HUD + 本地气泡 (RefCounted, ~178 行)
...
| `pet.gd` | ~300 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 4 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (时钟+气泡)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)。设置开关通过 `EventBus.setting_toggled` 同步到各子系统。**反重力系统**: `gravity_sign` 符号乘数统一控制所有垂直冲量方向 |
...
| `pet_hud.gd` | ~178 | **HUD + 本地气泡管理器** (RefCounted，从 pet.gd 拆分)。管理全息时钟 (AR 悬浮抖动 + 气泡避让自动隐藏) 和本地定向气泡 (每宠物独立堆叠，最多3条，弹簧弹入+淡出上飘)。反重力时自动切换上方/下方定位 |
| `pet_effects.gd` | ~98 | **视觉特效管理器** (RefCounted，从 pet.gd 拆分)。管理冲击波和全息拖影的数据生命周期和渲染委托。`update()` 返回是否有视觉变化供按需重绘，`render()` 接收 CanvasItem 执行实际绘制。未来新特效在此扩展 |
| `clone_pet.tscn` | — | 克隆分身预制体。与 `pet.tscn` 结构一致，但挂载 `clone_pet.gd` 脚本 |
| `clone_pet.gd` | ~29 | **克隆分身控制器**，继承 `pet.gd`。覆写 2 个方法实现功能裁剪：`_unhandled_input()` 恢复拖拽但屏蔽右键菜单、`_on_setting_toggled()` 响应通用设置但忽略原体专属功能 (HUD 时钟通过 `pet_hud.clock_enabled=false` 屏蔽)。克隆体完全自治 |
| `eye_behavior.gd` | ~193 | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式: 鼠标追踪 → 社交注视 → 好奇游走。含机械虹膜眨眼动画。**按需重绘**: `is_animating()` 基于每帧瞳孔位移量检测 |
...
pet.gd (核心控制器 ~300 行)
  ...
  ├── pet_hud.gd      (HUD+气泡, RefCounted, ~178 行)
...
**设计原则**：核心控制器 + 4个 RefCounted 子系统。子系统均通过 `xxx.pet = self` 注入宿主引用，不依赖场景树。克隆体通过继承+覆写裁剪功能，HUD 时钟通过 `pet_hud.clock_enabled=false` 控制。
