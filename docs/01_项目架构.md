...
| `context_menu/sysinfo_bubble.gd` | **系统信息气泡** (RefCounted, ~200行)。终端扫描报告风格 (`SYSTEM SCAN`)，RichTextLabel + BBCode 分区着色。**身份段** (HOST/USER/OS/DISP 青色标签) + **硬件段** (CPU/GPU/MEM/IP 蓝色标签) + **存储段** (磁盘竖排，三级颜色: 绿>30%/金10~30%/红<10%)。NVIDIA 独显优先。IP 过滤虚拟网卡 (Radmin/FlClash/蓝牙/APIPA)。视口边界钳制 + 弹簧动画 + overlay_rect 解除穿透 |
