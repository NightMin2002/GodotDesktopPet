...
| `pet.gd` | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染 (Canvas 绘图 API)。通过 9 个 RefCounted 子系统委托职责。内含瞳孔方向优先级决议层。提供 `visual_rotation` / `ag_flip` 计算属性统一处理反重力视觉翻转。支持屏幕穿越双重渲染幽灵副本 |
...
| `eye_behavior.gd` | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。`forced_look_dir` 作为最高优先级注视方向，由 pet.gd 决议层统一写入 (状态/游戏/全息屏等均不直接赋值)。支持休眠模式 (半闭眼)、扫描模式、检索动画、通用图标覆盖 (mail/alert/question/error)。好奇观察：即使追踪关闭，偶尔盯一眼鼠标 |
...
| `context_menu/sec_behavior.gd` | **行为分区 builder** (RefCounted)。窗口模式/行为指令/步态单选 + 模式开关 (指针跟踪/反重力/空间跳跃/屏幕穿越) + 踏板外观胶囊 + 指令序列调试面板 |
