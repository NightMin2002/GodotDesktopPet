...
│       ├── hud_panel.gd           ← 统一 HUD 悬浮面板 (RefCounted, ~257 行)
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `event_bus.gd` | ~35 | **全局事件总线**。声明了 16 个信号用于模块间解耦通信，覆盖拖拽、状态变化、UI 控制、系统功能 (含 `force_show_bubble` 强制气泡中断)、窗口模式切换、行为指令切换、克隆系统 (clone_pet/dismiss_clones) 等类别 |
...
| `hud_panel.gd` | ~257 | **统一 HUD 悬浮面板** (RefCounted)。宠物侧边悬浮的实时信息卡片。当前组件: 系统时钟 + WiFi SSID 显示。WiFi 通过 `WorkerThreadPool` 后台查询零卡顿，15秒刷新。点击 WiFi 行直接打开 Windows 网络设置。智能定位: 宠物在右半屏→面板放左边。右键菜单打开时自动淡出避让。各组件独立开关，全部关闭时面板自动隐藏 |
