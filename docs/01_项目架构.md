...
| `clone_manager.gd` | ~270 | **克隆与行为管理器** (从 main.gd 拆分)。管理克隆体生命周期 (召唤/遣散)、安静待命排队 (基于宠物直径的精确间距 + `reorganize_quiet_queue` 动态槽位分配，含 X/Y 双轴目标适配反重力)、行为指令切换。监听 `anti_gravity` 设置变更自动重新排队。**遣散和告别退出采用相同的仪式感动画**: 克隆体依次说告别语 → 滚向屏幕边缘 → 旋转淡出消失，间隔0.35s依次退场 |
...
| `idle.gd` | 待机 (1.0~4.0s)，结束时 85% 转 Walk / 15% 触发 Jump。注视同伴时不跳动 (检测 `eye_behavior._look_at_pet`)。散步者接近时被动跳开让路 (140px 检测范围，冲量 -350，0.5s 冷却)。安静模式下精确匹配 `retreat_target_x` 槽位坐标 (`_is_at_slot()` X+Y 双轴验证，适配反重力)，持续 lerp 微校正物理漂移 (速率8，约0.3秒归位)，高阻尼 (linear=5, angular=8) 锁定位置 |
...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。分组布局: 宠物功能开关 (眼球追踪/全息时钟/碎碎念) → 模式子菜单 (🌍 窗口模式 ▸ / 🏃 行为指令 ▸ / 🔮 模式 ▸) → 功能子菜单 (✨ 视觉特效 ▸ / 🎮 娱乐玩法 ▸ / ⏰ 提醒) → 分身操作 → 系统级 (自启动/退出) |
| `context_menu.gd` | 菜单 UI 逻辑。实现弹性跟随宠物 (Lerp)、Q 弹展开/收缩动画、自定义浮动 Tooltip。**级联子菜单系统**: 支持开关型 (`_create_submenu`) 和单选型 (`_create_radio_submenu`) 两种模式，hover 150ms 延时弹出 + 300ms 延时关闭 + 鼠标二次验证。窗口模式子菜单 (自由漫游/窗口封闭/窗口排斥)、行为指令子菜单 (自由行动/安静待命)、模式子菜单 (反重力等全局模式)、视觉特效子菜单 (冲击波/尾流)、娱乐玩法子菜单 (滚动散步)。单选子菜单带 tooltip 描述。碎碎念三态、外部点击关闭 (含子菜单区域排除) |
