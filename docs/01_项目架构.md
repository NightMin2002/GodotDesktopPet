...
| `pet.gd` | ~412 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 6 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)、`IdleBehaviors` (待机微行为)。**休眠挡板渲染**: 通过圆弧段多边形绘制上方机械挡板，实现半闭眼效果。高光反射点随 drowsy 程度淡出。所有 draw_circle 均启用抗锯齿 |
...
| `idle_behaviors.gd` | ~215 | **Idle 微行为管理器** (RefCounted)。管理待机状态下的 2 种微行为子阶段: **低功耗休眠** (鼠标静止 5 分钟触发，机械挡板半闭眼 + 瞳孔下垂 + 休眠气泡，鼠标活动后立即唤醒，本轮离席仅触发一次) / **系统自检** (真实时钟驱动，每 60~90 分钟一次，3小时内最多 3 次)。分身不运行。可被拖拽/戳一戳打断 |
