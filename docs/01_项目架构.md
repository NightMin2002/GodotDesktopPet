...
| `idle.gd` | 极短待机 (0.3~1.5s)，结束时 85% 转 Walk (小蹦跳)，15% 触发 Jump (大跳)。安静模式下检测是否到达边缘，未到位则重新触发 Retreat |
| `walk.gd` | **双模式移动**：80% 蹦跳 (向上冲量 300~500 + 水平 130~260，落地回 Idle) / 20% 滚动漫步 (持续扭矩 50000 滚到屏幕对面边缘，到达后回 Idle)。靠近边缘自动调转方向 |
