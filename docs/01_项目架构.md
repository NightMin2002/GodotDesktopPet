...
| `pet.gd` | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染 (Canvas 绘图 API)。通过 9 个 RefCounted 子系统委托职责：`PokeSystem`、`PetHUD`、`HudPanel`、`PetEffects`、`EyeBehavior`、`IdleBehaviors`、`FreeRoamSystem`、`PetGaming`、`PetMovement`。内含瞳孔方向优先级决议层。支持屏幕穿越双重渲染幽灵副本 |
