...
| `pet_holo_screen.gd` | **全息屏渲染控制器** (RefCounted)。管理全息迷你屏的梯形透视渲染 (近端收缩 15% + 微后仰)、投影支架线绘制、屏幕矩形计算 (供面板定位避让)。从 pet_gaming.gd 抽离，为后续多模式个人终端扩展做准备 |
...
触发方式: `IdleActivities` 自动调度 (权重 33%) + 指令序列手动触发。
