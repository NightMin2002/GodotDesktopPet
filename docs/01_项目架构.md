...
| 原则 | 正确做法 | 错误做法 |
|------|---------|---------|
| 用人类直觉的词 | "分身已部署。" (人人都懂"分身") | "实例副本已实例化。" (纯程序员黑话) |
| 技术感在句式，不在选词 | "信号同步中。" (句式冷淡，词汇易懂) | "IPC 握手协议执行中。" (故意炫技) |
...
│       ├── clone_pet.gd           ← 克隆分身控制器 (~23 行，继承 pet.gd, super 委托)
...
│
├── ui/                             ← 界面层
│   ├── context_menu.tscn          ← 右键菜单场景树 (6 个分区入口 + 退出)
│   ├── context_menu.gd            ← 右键菜单主控制器 (~1000行, 委托 5 个子系统)
│   ├── context_menu/              ← 右键菜单子系统 (RefCounted)
│   │   ├── info_sidebar.gd        ← 信息侧栏 (~199行)
│   │   ├── sysinfo_bubble.gd      ← 系统信息气泡 (~140行)
│   │   ├── submenu_system.gd      ← 级联子菜单 (~490行, L2+L3 两层级联)
│   │   ├── menu_tooltip.gd        ← 浮动提示 (~88行)
│   │   └── effect_preview.gd      ← 特效预览 (~225行, 注册表驱动)
...
| `clone_manager.gd` | ~160 | **分身与行为管理器** (从 main.gd 拆分)。最多 5 个分身 (总计 6 个宠物实例)，各自独立状态机+物理碰撞。**生命周期**: 启动时从 SettingsManager 恢复 clone_count 静默重建 → 右键菜单部署 (clone_scene.instantiate + 色偏分配) → 回收时 freeze + 禁碰 + 缩放淡出 + queue_free。安静待命排队、行为指令全员同步。每个克隆体拥有独立的 `PetColorPalette` 实例，通过 `_generate_distinct_hue()` 确保色调间距 ≥ 29° |
...
| `pet.gd` | ~740 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 7 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)、`IdleBehaviors` (待机微行为)、`FreeRoamSystem` (空间跳跃)。**屏幕穿越**: `_draw_body(world_offset)` 抽取本体绘制支持双重渲染幽灵副本 (本体+特效+弧线均偏移渲染)，`_update_screen_wrap()` 管理传送和拖影坐标同步，退场冻结时自动跳过传送+清除幽灵偏移。指令序列触发支持 `"hibernate:N"` 格式解析。所有 draw_circle 均启用抗锯齿 |
| `free_roam.gd` | ~460 | **空间跳跃系统** (RefCounted，从 pet.gd 拆分)。管理踏板跳跃、横移、电梯下降、破碎效果的全部自由移动逻辑。**概率决策**: 落稳后 40%继续跳/25%踏板横移/25%跳下破碎/10%电梯。**屏幕自适应**: `screen_scale()` 基准 1080p。**踏板横移 (phase 4)**: 空气墙防掉落 + 踏板 lerp 跟随 + 0.4s 保护期。**跳下破碎**: 蓄力→起跳→踏板闪白+6碎片飞散 (碎片从宠物起跳位置飞散，颜色通过 `palette.shift_color()` 联动宠物体色)。**电梯下降**: 关重力+匀速下移+刚性锁定。**屏幕穿越适配**: `_detect_wrap()` 检测单帧位移>40%屏幕宽，穿越时 `_respawn_platform_at_pet()` 在新位置重建踏板，旧踏板保留自然淡出。**踏板生命周期**: 3.5s 定时销毁，宠物在场时延期重检。内含 `PlatformVisual` 渲染类 (展开动画+刻度线+能量粒子+着陆闪光) |
...
| `hud_panel.gd` | ~397 | **统一 HUD 悬浮面板** (RefCounted)。宠物侧边悬浮的实时信息卡片。当前组件: 系统时钟 + WiFi SSID 显示。**双显示模式**: 常驻模式 (`hud_pin`) 面板始终可见；悬浮模式 (默认) 鼠标悬停宠物时淡入、离开后 0.3 秒延迟淡出。**WiFi SSID 修正**: `Get-NetConnectionProfile.Name` 返回的是配置文件名 (可能带数字后缀)，通过 `netsh wlan show profiles` 获取已保存 SSID 列表并反向匹配最长前缀，还原真实 SSID。`WorkerThreadPool` 后台查询零卡顿，15秒刷新，`_wifi_in_flight` 标志防止并发查询。**悬浮模式优化**: 后台查询结果不受面板可见性限制，避免 pending 数据被卡住；面板重新显示时立即触发刷新确保数据最新。点击 WiFi 行直接打开 Windows 网络设置 (指针手型光标)。智能定位: 宠物在右半屏→面板放左边。右键菜单打开时自动淡出避让。各组件独立开关，全部关闭时面板自动隐藏。两行布局统一为 `[●标签列(38px)] + [值文本(17px)]` 结构确保对齐。**边框色联动**: 监听 `pet_color_changed` 信号实时更新面板边框色 |
| `pet_effects.gd` | ~268 | **视觉特效管理器** (RefCounted，从 pet.gd 拆分)。管理冲击波、全息拖影和静电弧。**双配色模式**: 虹彩 (彩虹循环, 默认) / 跟随体色 (纯色)。**静电弧**: 多宠物距离<150px时自动绘制锯齿状闪电弧，**噪声缓存架构** (`_arc_jitters` 字典存归一化噪声 -1~1，0.07s 周期重生形状；`_build_arc_path()` 每帧从实时 `global_position` + 缓存噪声重建路径坐标)，彻底消除高速运动时弧线起点偏移。双色渐变辉光+白色内核，`_arc_flash` 统一闪烁衰减。坐标系采用世界对齐空间 (`draw_set_transform` 反旋转)，确保宠物滚动时弧线不偏移。**退场过滤**: freeze=true 的宠物自动排除出弧线检测和渲染，避免退场动画期间弧线闪现。**屏幕穿越适配**: `_arc_distance()` / `_arc_offset()` 取直接和环绕距离中的最短路径，弧线可跨边界连接。颜色通过 `palette.effective_hue()` 联动 |
...
| `clone_pet.gd` | ~23 | **克隆分身控制器**，继承 `pet.gd`。覆写 `_unhandled_input()` (剥离右键菜单) + `_on_setting_toggled()` (黑名单拦截 HUD 专属设置后 `super` 委托，新增通用设置时只需修改 pet.gd)。保留: 物理碰撞/状态机/眼球追踪/拖拽/本地气泡(偏色边框)/特效(偏色) |
| `eye_behavior.gd` | ~370 | **瞳孔行为控制器** (RefCounted)。三种瞳孔模式 + 机械虹膜眨眼动画。**强制注视方向**: `forced_look_dir` 由状态机设定，优先级最高。**休眠模式**: `drowsy_amount` 控制持续半闭状态 (lerp 过渡)，休眠态抑制正常眨眼。**扫描模式**: 1.2s 内 2 次快速左右来回。**检索动画**: `scanning_blend` + `scanning_done_blend` 双参数交叉混合。**瞳孔图标覆盖**: `eye_icon_blend`/`eye_icon_type`/`eye_icon_time` 独立于检索动画的通用图标系统，支持 mail/alert/question/error。**好奇观察指针**: 即使追踪关闭，15% 概率偶尔盯一眼鼠标 2~4 秒 (缓慢跟随，lerp 4.0)。检索/图标期间抑制眨眼。提供 `show_eye_icon()`/`hide_eye_icon()` 接口。鼠标空闲时间 `_mouse_idle_time` 供休眠触发判断。按需重绘 |
...
| 文件 | 职责 |
|------|------|
| `context_menu.tscn` | 右键菜单场景树。**分区级联布局**: 主菜单仅 6 个分区入口按钮 (宠物/显示/行为/视觉/玩法/系统) + 退出按钮。每个分区按钮带彩色左边框徽章样式，hover/点击后向侧边展开 L2 分区面板，L2 内的子项可继续向外展开 L3 选项面板。**视觉规范**: 开关项右侧 `[●]`/`[○]`、子菜单右侧 `[+]`、面板入口胶囊按钮 (半透明背景+细边框+圆角)、退出红色胶囊 |
| `context_menu.gd` | 菜单主控制器 (~1000行)。**分区级联架构**: 主菜单 (L1, 6 个分区入口) + L2 分区面板 (动态代码构建) + L3 选项面板 (单选/开关/操作) + 信息侧栏。**方向感知三级定位**: `_menu_side` 全局方向状态驱动主菜单/侧栏/L2/L3/tooltip 的弹出方向和动画锚点，三级面板依次向外级联。通过 5 个 RefCounted 子系统委托: `_sidebar` (侧栏), `_sysinfo_bubble` (系统气泡), `_submenu` (级联子菜单), `_tooltip` (浮动提示), `_fx_preview` (特效预览)。**6 个分区构建函数** (`_build_sec_pet/_display/_behavior/_visual/_play/_system`) 动态创建 L2 面板内容和 L3 子菜单。**宠物分区**: 碎碎念 + 分身管理 (L3 操作面板) + 提醒管理 (胶囊按钮)。**胶囊按钮**: 面板入口 (外观主题/提醒管理/踏板外观) 和退出按钮使用 `_apply_capsule_style()`。**指令序列**: 宠物状态预览机制 (L3 子菜单)。**深夜模式指令拒绝**: `_on_debug_behavior_pressed()` 检测 `nighttime_mode`，拒绝执行并冒泡提示 |
| `context_menu/info_sidebar.gd` | **信息侧栏** (RefCounted, ~204行)。日期/时钟/WiFi/电池/开机时长。**方向感知定位**: 读取 `_menu._menu_side` 决定侧栏在主菜单左/右侧。WiFi+电池+CPU+RAM 合并一次 PS 调用，WiFi 同样采用 `netsh wlan show profiles` 反向匹配真实 SSID，开机时长本地计算零开销。`WorkerThreadPool` 异步执行 |
...
| `context_menu/submenu_system.gd` | **级联子菜单** (RefCounted, ~490行)。**双层级联架构**: L2 (分区面板) + L3 (选项面板) 独立管理。开关子菜单 (`create_toggle`) + 单选子菜单 (`create_radio`) 均支持 `level` 参数指定层级。**方向感知三级级联**: L3 基于 L2 面板边缘继续向外定位。**hover 互不干扰**: L3 打开时不触发 L2 关闭，鼠标在 L2 面板时不关闭 L3。`get_all_visible_panels()` 供外部点击检测覆盖全层级 |
| `context_menu/menu_tooltip.gd` | **浮动提示** (RefCounted, ~88行)。子菜单项的描述性浮动提示，**方向感知弹出**: 读取 `_menu._menu_side` 联动级联方向 |
| `context_menu/effect_preview.gd` | **特效预览** (RefCounted, ~225行)。特效开关项的 hover 动画预览面板。**注册表驱动**: `_entries` 字典以 setting_id 为 key 统一管理 panel/ctrl/btn，`_register()` 一行注册新预览。**三种内嵌渲染类**: `ShockwavePreview` (同心冲击波扩散) / `TrailPreview` (正弦轨迹虹彩拖影) / `ArcPreview` (双宠物锯齿闪电弧)。通用面板创建/弹簧动画/方向感知定位全封装，`context_menu.gd` 仅保留 4 行引用 |
| `reminder_panel.gd` | **提醒管理面板** (CanvasLayer, ~740行)。**独立窗口模式**: 宠物附近弹出、可拖拽标题栏 (HBoxContainer + Label + x 关闭按钮)、边界钗制。打开时 `get_window().grab_focus()` 确保 OS 键盘焦点 (解决 WS_EX_TOOLWINDOW 透明窗口焦点丢失)。自适应高度卡片列表 (hover 高亮+删除淡出动画)、自定义拨轮时间选择器 (▲/当前值/▼ + 鼠标滚轮 + 上下拖拽, 分钟 5 分钟步进)、`1x单次`/`↻ 每日` 胶囊切换、回车提交。**定位策略**: 宠物在右半屏→面板弹到左侧，反之右侧，边界钗制防溢出。**UI 主题色同步**: 监听 `ui_theme_changed` 实时更新面板边框/标题/分隔线/拨轮底板/类型标签。外部点击关闭时 `set_input_as_handled()` 防穿透。一次性提醒触发后自动删除，每 10 秒检查系统时间 |
...
| `theme_panel.gd` | **外观主题面板** (CanvasLayer, ~625行)。**独立窗口模式**: 宠物附近弹出、可拖拽标题栏 (HBoxContainer + Label + x 关闭按钮)、边界钗制。打开时 `get_window().grab_focus()` 确保 OS 键盘焦点。Canvas 绘制 HSV 色轮 + 拖拽选色 + 目标选择器 (原体/分身/界面) + S/V 滑块 + 8 个预设色 + 重置。撞色检测提示 (色调距离 < 6%/10%)。**定位策略**: 与提醒面板一致，宠物在右半屏→面板弹到左侧。面板边框实时跟随 UI 主题色。颜色变更通过 `EventBus.pet_color_changed` / `ui_theme_changed` 广播 + `SettingsManager` 持久化 |
| `platform_style_panel.gd` | **踏板外观面板** (CanvasLayer, ~470行)。**独立窗口模式**: 宠物附近弹出、可拖拽标题栏。定位策略与提醒/主题面板一致。4 张风格卡片带**实时动画预览** (能量束/脉冲链/极简/随机)。内嵌类 `_StylePreview` (Control) 每帧 `queue_redraw()` 绘制缩小版踏板动画，关闭时 `set_process(false)` 节省性能。随机卡片用三段混合展示。点击卡片切换风格，选中态边框高亮 + ●/○ 状态。主题色同步联动预览颜色/边框。入口为模式子菜单底部胶囊按钮 |
