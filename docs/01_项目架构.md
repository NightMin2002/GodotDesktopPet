...
| `pet.gd` | ~280 | **宠物核心控制器**。管理 FSM 状态机调度、输入事件路由、科幻单眼渲染。通过 5 个 RefCounted 子系统委托各职责: `PokeSystem` (戳一戳)、`PetHUD` (本地气泡)、`HudPanel` (统一 HUD 悬浮面板)、`PetEffects` (冲击波+拖影)、`EyeBehavior` (瞳孔)。设置开关通过 `EventBus.setting_toggled` 同步到各子系统。每帧转发鼠标悬浮状态至 HUD 面板。**反重力系统**: `gravity_sign` 符号乘数统一控制所有垂直冲量方向 |
